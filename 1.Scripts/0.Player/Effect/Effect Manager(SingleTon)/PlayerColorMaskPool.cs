...
public class PlayerColorMaskPool : MonoBehaviour
{
    ...
    #region SingleTon
    public static PlayerColorMaskPool instance;
    ...
    public GameObject GetFromPool(string name, Vector3 worldPosition, Quaternion rotation)
    {
        // 큐에 없으면 새로 추가
        Queue<GameObject> poolQueue = poolDictionary[name];
        if (poolQueue.Count <= 0)
        ...
        Vector2 ViewportPosition = CustomCamera.WorldToViewportPoint(worldPosition);
        ...
        //Set PlayerColorMask Fixed Position
        PlayerColorMask playerColorMask = objectToSpawn.GetComponent<PlayerColorMask>();
        playerColorMask.CanvasRect = CanvasRect;
        playerColorMask.worldPosition = worldPosition;
